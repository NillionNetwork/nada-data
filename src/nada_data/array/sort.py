...
def odd_even_sort(values: List[secret_int], ascending: bool, lo: int, n: int):
    if n > 1:
        m = int(n / 2)
        odd_even_sort(values, ascending, lo, m)
        odd_even_sort(values, ascending, lo + m, m)
        _odd_even_merge(values, ascending, lo, n, 1)


def sort_nada_array(values: List[secret_int], ascending: bool = True):
    """
    Sort the contents of :values: in either ascending or descending order
    """
    odd_even_sort(values, ascending, 0, utils.next_power_of_two(len(values)))
    return values
